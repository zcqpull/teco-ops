...
malloc_free文件夹为用户展示了`malloc`和`free`接口的使用方式，示例展示了如何通过`malloc`在SPM内存中申请堆内存空间，通过`free`释放在SPM中申请的堆内存空间。
...
该文件夹内的示例主要涉及`malloc`和`free`接口，您可以通过SDAA C编程模型中的SPM内存分配和释放章节学习接口的使用方法：
- [SDAA C编程指南-SPM内存分配和释放](http://docs.tecorigin.com/release/sdaac/v1.16.0/#5bezap6xl93vzz)：介绍SPM内存分配与释放的各个接口。
...
```
bash run_customops.sh malloc_free
```
