...
RMA文件夹为用户展示了数据搬运操作中RMA数据搬运接口的使用方式，示例展示了阻塞型RMA数据搬运接口及非阻塞型RMA数据搬运接口的使用方式。
...
该文件夹内的示例主要涉及SDAA C编程模型所提供的RMA数据搬运接口，您可以通过SDAA C编程模型中的RMA数据搬运章节学习RMA数据搬运接口的使用方法：
- [SDAA C编程指南-RMA数据搬运](http://docs.tecorigin.com/release/sdaac/v1.16.0/#14iquedp6b414x)：介绍RMA数据搬运的各个接口。
